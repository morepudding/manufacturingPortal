/**
 * Test Customer Order avec OrderNo connu
 */
...
async function testCustomerOrderByOrderNo() {
  console.log('🔍 Testing Customer Order Line by OrderNo...\n');
  
  const token = await getAccessToken();
  console.log('✅ Token obtained\n');

  return new Promise((resolve) => {
    const filter = "OrderNo eq 'C1000038587' and LineNo eq '1'";
    const select = 'OrderNo,LineNo,PartNo,CHullNumber,CustomerNo,ConfigurationId';
      ...
      res.on('end', () => {
        ...
        try {
          ...
          if (data.value && data.value.length > 0) {
            ...
            console.log('✅ Service Customer Order fonctionne!\n');
            resolve(true);
          } else {
...
(async () => {
  try {
    const success = await testCustomerOrderByOrderNo();
    console.log(success ? '✅ Test passed - Service ready!' : '❌ Test failed');
