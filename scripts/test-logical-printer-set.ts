/**
 * Script de test : LogicalPrinterSet Endpoint
 * 
 * Objectif: Tester l'endpoint PrintDialog.svc/LogicalPrinterSet
 * avec connexion directe IFS Cloud (ancien système)
 * 
 * Usage: npx tsx scripts/test-logical-printer-set.ts
 */
...
/**
 * Étape 2 : Interroger l'endpoint LogicalPrinterSet
 */
async function getLogicalPrinters(accessToken: string): Promise<IFSPrinter[]> {
  console.log('\n🖨️  [2/3] Récupération LogicalPrinterSet...')
  
  const endpoint = `${IFS_CONFIG.baseUrl}/PrintDialog.svc/LogicalPrinterSet`
...
function displayPrinters(printers: IFSPrinter[]): void {
  ...
  console.log('-'.repeat(100))
  console.log('N° | LogicalPrinter | Description | PhysicalPrinter | Autres champs')
  console.log('-'.repeat(100))

  printers.forEach((printer, index) => {
    const logicalPrinter = printer.LogicalPrinter || 'N/A'
    const description = printer.Description || 'N/A'
    const physicalPrinter = printer.PhysicalPrinter || 'N/A'
    
    // Compter les champs supplémentaires
    const mainFields = ['LogicalPrinter', 'Description', 'PhysicalPrinter']
    const otherFields = Object.keys(printer).filter(key => 
      !mainFields.includes(key) && !key.startsWith('@')
    )
    
    console.log(
      `${(index + 1).toString().padEnd(2)} | ` +
      `${logicalPrinter.padEnd(14)} | ` +
      `${description.substring(0, 25).padEnd(25)} | ` +
      `${physicalPrinter.substring(0, 15).padEnd(15)} | ` +
      `${otherFields.length} champs`
    )
  })
  ...
  console.log('\n🔑 Tous les champs disponibles dans LogicalPrinterSet:')
  ...
  const withPhysical = printers.filter(p => p.PhysicalPrinter).length
  console.log(`   Avec PhysicalPrinter: ${withPhysical}/${printers.length}`)
}
 ...
 */
async function main() {
  console.log('🚀 Test LogicalPrinterSet - IFS Cloud Direct\n')
